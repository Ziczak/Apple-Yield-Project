...
from tensorflow.keras.layers import Conv1D, MaxPooling1D, Flatten, Dense, LeakyReLU, Dropout
...
final_model = Sequential([
    Conv1D(filters=64, kernel_size=3, activation='relu', input_shape=(X_scaled.shape[1], X_scaled.shape[2])),
    MaxPooling1D(pool_size=2),
    Dropout(0.2),
    Conv1D(filters=32, kernel_size=3, activation='relu'),
    MaxPooling1D(pool_size=2),
...
plt.title('CNN Predictions vs Actual vs Future Predictions')
plt.xlabel('Year')
plt.ylabel('Yield')
plt.legend()
plt.grid()
